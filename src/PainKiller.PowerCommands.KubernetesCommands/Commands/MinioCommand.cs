...
public class MinioCommand : CommandBase<PowerCommandsConfiguration>
{
    ...
    public override RunResult Run()
    {
        ...
        else
        {
            ...
            //docker run -p 9000:9000 -p 9090:9090 --name minio1 -v D:\minio\data:/data -e "MINIO_ROOT_USER=admin" -e "MINIO_ROOT_PASSWORD=abc123" quay.io/minio/minio:latest server /data --console-address ":9090"
            ShellService.Service.Execute("docker",$"run -p 9000:9000 -p 9090:9090 --name {containerName} -v {path}:/data -e \"MINIO_ROOT_USER=minioadmin\" -e \"MINIO_ROOT_PASSWORD=minioadmin\" quay.io/minio/minio:latest server /data --console-address \":9090",CdCommand.WorkingDirectory, WriteLine,"", waitForExit: true);
    ...
    {
        ShellService.Service.Execute("docker","stop minio1",CdCommand.WorkingDirectory, WriteLine,"", waitForExit: true);
        WriteSuccessLine("MinIO container minio1 stopped.");
        ShellService.Service.Execute("docker","rm minio1",CdCommand.WorkingDirectory, WriteLine,"", waitForExit: true);
